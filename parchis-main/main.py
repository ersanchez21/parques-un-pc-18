...
class jugador:
    ...
    def __init__(self, nombre, color, fichas, GanoJugador=False, UltimaFicha=None):
        """
        Crea los jugadores y sus respectivas fichas según el número de jugadores ingresado.

        @param tablero: Lista con las posiciones iniciales de las fichas.
        @param numeroJugadores: Cantidad de jugadores en la partida.
        @param nombres: Lista con los nombres de los jugadores.

        
        @return: Lista de objetos 'jugador', cada uno con sus fichas.
        """
        self.nombre = nombre
        self.color = color
        self.fichas = fichas
    ...
    def TirarUnDado(self,desa):  # Para saber quien empieza
        ...
        """
        @param desarrollador: Si el usuario ingresa el valor del dado, este parámetro es verdadero.  
        @return: Número aleatorio entre 1 y 6, que representa el lanzamiento del dado.  
...
class espacio(object):
    ...
    """
   Representa cada espacio en el tablero. Considera todos los espacios del tablero, incluyendo los espacios especiales (salidas, espacios seguros, espacios de llegada).

    """

    def __init__(self, numeroEspacio, etiqueta,x,y,tipoEspacio="normal", colorCasillaEspecial="ninguno",orientacion="Ninguna"):
        """
       @param nombre: Entero entre 1 y 101 que representa un espacio en el tablero.  
    @param tipo: Tipo de espacio (String) [inicio (requiere color), llegada, especial (requiere color), seguro, salida].  
    @param color: (String) Representa los espacios de llegada para cada jugador.  
    @return: None 
        """
        global mod
...
def CrearTablero():
    """
    Crea el tablero de juego, que está compuesto por:
    68 espacios comunes,
    7 espacios para cada color (28 en total),
    4 espacios de inicio y un espacio de llegada.
    """
    global mod
    tablero = []
    for x in range(68):
...
def CrearJugadoresYFichas(tablero, numeroJugadores, nombres):
    ...
    for x in range(numeroJugadores):  # Itera en el número de jugadores que hayan ingresado
        ...
        if x == 0:  # Si x es 0, sera el primer jugador, y por ende fichas rojas
            """
            68 -> Posición de inicio para rojo.
            69 -> Posición de inicio para verde.
            70 -> Posición de inicio para amarillo.
            71 -> Posición de inicio para azul. 
            """
...
def pedirDatos():
    """
    Pide al usuario que ingrese el número total de jugadores y luego solicita el nombre de cada jugador.
    """
...
def ObtenerMayor(listaJugadores):
    """
    Obtiene el valor más alto de los jugadores basado en el atributo 'valor'.

    @param listaJugadores: lista de elementos de clase 'jugador'
...
def OrdenDeJuego(ListaJugadores, modoDesarrollador=False):
    ...
    Dos jugadores han ingresado:

    Primer jugador: 3
    Segundo jugador: 6
    El segundo jugador comienza el juego y los demás jugadores juegan en el mismo orden de la lista de jugadores.

    @param ListaJugadores: Lista de objetos de la clase 'jugador'
    @param modoDesarrollador: Solo para fines de evaluación.
    """
    global desa,mod
    aux = ListaJugadores[:]
    while (len(aux) != 1):
...
def GameOver(Jugadores):
    """
    @param Jugadores: Lista de objeto de clase 'Jugador'
...
def posiblesMovimientos(JugadorActual, resultadoDado, ListaJugadores):
    """
    Devuelve una lista de todos los movimientos posibles de un jugador dado el resultado de un dado.

    @param JugadorActual: Objeto de la clase 'Jugador'.
    @param resultadoDado: El número de espacios a avanzar.
    @param ListaJugadores: Lista de objetos de la clase 'Jugador'
    """
    ...
    diccionarioSeguros = {"rojo": 68, "verde": 17, "amarillo": 34,
                          "azul": 51}  
    # Seguros donde inician las casillas especiales de cada color
...
def opciones(Lista, JugadorActual):
    ...
    """
    Solicita el número de jugadores y sus nombres, manejando distintos modos de entrada.
